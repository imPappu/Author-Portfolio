<?php
/**
 * Single Book Template (Liquid Edition - Dynamic Platforms)
 *
...
while ( have_posts() ) : the_post();
    $custom_title = get_post_meta(get_the_ID(), 'n_book_custom_title', true);
    $display_title = !empty($custom_title) ? $custom_title : get_the_title();
    $description = get_post_meta(get_the_ID(), 'n_book_description', true);
    $custom_cover = get_post_meta(get_the_ID(), 'n_book_cover_url', true);
    $purchase_links = get_post_meta(get_the_ID(), 'n_purchase_links', true) ?: [];
?>

<article id="book-<?php the_ID(); ?>" <?php post_class('book-detail-wrapper'); ?>>
    <div class="container">
        <div class="book-layout">
            ...
            <div class="book-info">
                <span class="pill-tag fade-in-ready">SELECTED WORK</span>
                ...
                <div class="book-description fade-in-ready">
                    <?php echo !empty($description) ? wpautop(wp_kses_post($description)) : '<p>No description provided.</p>'; ?>
                </div>

                <?php if(!empty($purchase_links)): ?>
                <div class="buy-section fade-in-ready">
                    <h4 style="margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px; font-size: 0.8rem;">Acquire Your Copy</h4>
